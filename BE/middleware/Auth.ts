...
export default class Auth {
    ...
    public static requireAuthetication(req: { cookies: { jwt: any; }; }, res: any, next: () => void) {
    ...
    public static requireAuthorization(authLevel: Role) {
        ...
        return (req: { path: any; method: any; }, res: { locals: { user: { role: string | number; _id: any; }; }; sendStatus: (arg0: number) => void; }, next: () => void) => {
            ...
            if (LEVELS[res.locals.user.role].includes(authLevel)) {
                ...
                if (process.env.NODE_ENV !== 'test') {     
                    console.info(`\x1b[36m\[server]: User:\x1b[35m${res.locals.user._id}\x1b[36m\ with authLevel:\x1b[35m${res.locals.user.role}\x1b[36m\ is authorized, now accesing:\x1b[32m${req.path}\x1b[0m\x1b[36m\, methdod:\x1b[35m${req.method}\x1b[0m`)
                }
                next()
            } else {
                res.sendStatus(403)
